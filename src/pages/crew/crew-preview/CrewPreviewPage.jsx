...
import './crewPreveiwStyle.css'
import { formatDate } from '@/utils/dateHelper';

import AvatarIcon from '/assets/images/user.png';
const CrewPreviewPage = ({ response }) => {
  const title1 = (
  ...
  const [details5, setDetails5] = useState([
    { title: 'Pin Code', name: response?.presentAddress.pinCode?response?.presentAddress.pinCode:'---' },
    { title: 'Country', name: response?.presentAddress?.country?response?.presentAddress?.country:'---' },
    { title: 'State', name: response?.presentAddress?.stateId?.name?.english?response?.presentAddress?.stateId?.name?.english +' ['+response?.presentAddress?.stateId?.name?.hindi +']':'---' },
    { title: 'District', name: response?.presentAddress?.districtId?.name?.english?response?.presentAddress?.districtId?.name?.english+' ['+response?.presentAddress?.districtId?.name?.hindi+']':'---' },
    
    { title: 'Address 1', name: response?.presentAddress?.address1?response?.presentAddress?.address1:'---' },
    
    { title: 'Address 2', name: response?.presentAddress?.address2?response?.presentAddress?.address2:'---' }
    
  ]);
  const [details6, setDetails6] = useState([
    { title: 'Pin Code', name: response?.permanentAddress.pinCode?response?.permanentAddress.pinCode:'---' },
    { title: 'Country', name: response?.permanentAddress?.country?response?.permanentAddress?.country:'---' },
    { title: 'State', name: response?.permanentAddress?.stateId?.name?.english?response?.permanentAddress?.stateId?.name?.english +' ['+response?.presentAddress?.stateId?.name?.hindi +']':'---' },
    { title: 'District', name: response?.permanentAddress?.districtId?.name?.english?response?.permanentAddress?.districtId?.name?.english+' ['+response?.presentAddress?.districtId?.name?.hindi+']':'---' },
    
    { title: 'Address 1', name: response?.permanentAddress?.address1?response?.permanentAddress?.address1:'---' },
    
    { title: 'Address 2', name: response?.permanentAddress?.address2?response?.permanentAddress?.address2:'---' }
    ]);
  ...
  useEffect(() => {
    ...
    setDetails5([
      { title: 'Pin Code', name: response?.presentAddress.pinCode?response?.presentAddress.pinCode:'---' },
      { title: 'Country', name: response?.presentAddress?.country?response?.presentAddress?.country?.name?.english:'---' },
      { title: 'State', name: response?.presentAddress?.stateId?.name?.english?response?.presentAddress?.stateId?.name?.english +' ('+response?.presentAddress?.stateId?.name?.hindi +')':'---' },
      { title: 'District', name: response?.presentAddress?.districtId?.name?.english?response?.presentAddress?.districtId?.name?.english+' ('+response?.presentAddress?.districtId?.name?.hindi+')':'---' },
    ...
    setDetails6([
      { title: 'Pin Code', name: response?.permanentAddress.pinCode?response?.permanentAddress?.pinCode:'---' },
      { title: 'Country', name: response?.permanentAddress?.country?response?.permanentAddress?.country?.name?.english:'---' },
      { title: 'State', name: response?.permanentAddress?.stateId?.name?.english?response?.permanentAddress?.stateId?.name?.english +' ('+response?.presentAddress?.stateId?.name?.hindi +')':'---' },
      { title: 'District', name: response?.permanentAddress?.districtId?.name?.english?response?.permanentAddress?.districtId?.name?.english+' ('+response?.presentAddress?.districtId?.name?.hindi+')':'---' },
      ...
      setDetails7([
        { title: 'Licence Type', name: response?.licenseType?response?.licenseType:'---' },
        { title: 'Licence No.', name: response?.licenseNumber?response?.licenseNumber:'---' },
        { title: 'Issue Date', name: response?.licenseDate?.issue?formatDate(response?.licenseDate?.issue, 'date') :'---' },
        { title: 'Expiry Date', name: response?.licenseDate?.expiry?formatDate(response?.licenseDate?.expiry, 'date') :'---'  },
        ]);
        setDetails8([
          { title: 'Health Test Valid Upto', name: response?.test?.healthValidity?formatDate(response?.test?.healthValidity, 'date'):'---' },
          { title: 'Eye Test Valid Upto', name: response?.test?.eyeValidity?formatDate(response?.test?.eyeValidity, 'date'):'---' },
          ]);
  ...
  return (
    <div className="overview-main-container">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-[18px]">
        ...
        <TextDetailsTooltip details={details6} title={title6} maxLength='25'/>
        </div>
        <div className='section-shadow'><TextDetailsTooltip details={details7} title={title7} maxLength='25'/></div>
        <div className='section-shadow'><TextDetailsTooltip details={details8} title={title8} maxLength='25'/></div>
...
export default CrewPreviewPage;
