...
export default function CitizenComplainPage() {
  const { setCount } = useContext(CounterContext);

  useEffect(() => {
    setCount('Citizen Complain Page');
  }, []);
  ...
  const fetchRoutes = async ({ page, pageSize, search, sortBy, order }) => {
    const response = await apiService.get(`/v1/citizen-user/complain`, {
      params: {
        limit: pageSize,
        page: page + 1,
        search,
  ...
  return (
    <PaginatedTableView
      queryKey="citizen-complain"
      queryFn={fetchRoutes}
      columns={columns}
      pageName="Citizen Complain"
