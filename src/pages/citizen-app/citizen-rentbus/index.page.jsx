...
export default function CitizenRentBusPage() {
  const { setCount } = useContext(CounterContext);

  useEffect(() => {
    setCount('Citizen Rent Bus Page');
  }, []);
  ...
  const fetchRoutes = async ({ page, pageSize, search, sortBy, order }) => {
    const response = await apiService.get(`/v1/citizen-user/rent-bus`, {
      params: {
        limit: pageSize,
        page: page + 1,
        search,
  ...
  return (
    <PaginatedTableView
      queryKey="citizen-rentbus"
      queryFn={fetchRoutes}
      columns={columns}
      pageName="Citizen Rent a Bus"
