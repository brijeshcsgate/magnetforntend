...
const organisationStructureSchema = Yup.object().shape({
  englishName: Yup.string().trim().required(COMMON_SCHEMA),
});

const AddEditOrganisationStructure = () => {
  ...
  useEffect(() => {
    if (id) {
      setLoading(true);
      getApi(APIS.DESIGNATION, id)
        .then((res) => {
          const editData = res?.data?.designation;
          setData((prev) => ({
            ...prev,
  ...
  const handleSubmit = (values, { setSubmitting, resetForm }) => {
    ...
    if (id) {
      payload._id = id;
      patchApi(APIS.DESIGNATION, id, payload).then(() => {
        toast.success('Data updated successfully');
        navigate(-1);
      });
    } else {
      postApi(APIS.DESIGNATION, payload)
        .then(() => {
  ...
  return (
    <Formik
      enableReinitialize
      initialValues={data}
      validationSchema={organisationStructureSchema}
            ...
            {loading ? (
              ...
              <>
                <Header>
                  <div>
                    <BreadCrumbs
                      ...
                      boldItem={'Organisation Structure'}
                    />
                    <Heading>
                      {id ? 'Edit' : 'Add'} Organisation Structure
                    </Heading>
                  </div>
                  <ButtonContainer>
                    <Button
                ...
                <Content>
                  <Section>
                    <SidePanel
                      title={`Add Organisation Structure (En)`}
                      description={`Enter Organisation Structure Detail`}
                    />
                    <InputsContainer>
                      <div className="flex gap-8">
                        <div className=" pb-4 md:w-1/3">
                          <FormikTextField
                            label="Organisation Structure Name"
                      ...
                      <div className=" pb-4 md:w-1/3">
                        <FormikTextField
                          label="Organisation Structure Short Name"
                          placeholder="Enter Organisation Structure Short Name"
                  ...
                  <Section>
                    <SidePanel
                      title={`Add Organisation Structure (Hi)`}
                      description={`Enter Organisation Structure Detail`}
                    />
                    <InputsContainer>
                      <div className=" pb-4 md:w-1/3">
                        <FormikTextField
                          label="Organisation Structure Name"
                          placeholder="Enter Organisation Structure Name"
                          name="hindiName"
                          onChange={(e) => {
                            let value = e.target.value;
                            setFieldValue(
...
export default AddEditOrganisationStructure;
