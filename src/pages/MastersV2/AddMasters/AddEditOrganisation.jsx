...
const AddEditOrganisation = () => {
  ...
  useEffect(() => {
    if (id) {
      setLoading(true);
      getApi(APIS.ORGANISATION, id)
        .then((res) => {
          const editData = res?.data?.organization;
          setData((prev) => ({
            ...prev,
  ...
  const handleSubmit = (values, { setSubmitting, resetForm }) => {
    ...
    if (id) {
      payload._id = id;
      patchApi(APIS.ORGANISATION, id, payload)
        .then(() => {
      ...
      postApi(APIS.ORGANISATION, payload)
        .then(() => {
            ...
            {loading ? (
              ...
              <>
                <Header>
                  <div>
                    <BreadCrumbs
                      ...
                      boldItem={'Organisation'}
                    />
                    <Heading>{id ? 'Edit' : 'Add'} Organisation</Heading>
                  </div>
                  <ButtonContainer>
                    <Button
                ...
                <Content>
                  <Section>
                    <SidePanel
                      title={`Add Organisation  (En)`}
                      description={`Enter Organisation  Detail`}
                    />
                    <InputsContainer>
                      ...
                      <div className=" pb-4 md:w-1/3">
                        <FormikTextArea
                          ...
                          onChange={(e) => {
                            let value = e.target.value;
                            setFieldValue('address', startSpcaeRemover(value));
                          }}
                        />
                      </div>
                    </InputsContainer>
