...
const AddEditUserRole = () => {
  ...
  const handleSubmit = (values, { setSubmitting, resetForm }) => {
    ...
    if (id) {
      payload._id = id;
      patchApi(APIS.USER_ROLE, id, payload).then(() => {
        toast.success("Data updated successfully");
        navigate(-1);
      });
    } else {
      postApi(APIS.USER_ROLE, payload)
        .then(() => {
            ...
            {loading ? (
              ...
              <>
                <Header>
                  <div>
                    <BreadCrumbs
                      ...
                      boldItem={"User Role"}
                    />
                    <Heading>{id ? "Edit" : "Add"} User Role</Heading>
                  </div>
                  <ButtonContainer>
                    <Button
                ...
                <Content>
                  <Section>
                    <SidePanel
                      title={`Add User Role (En)`}
                      description={`Enter User Role Detail`}
                    />
                    <InputsContainer>
