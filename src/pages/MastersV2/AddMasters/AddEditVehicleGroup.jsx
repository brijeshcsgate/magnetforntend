...
const AddEditVehicleGroup = () => {
  ...
  useEffect(() => {
    if (id) {
      setLoading(true);
      getApi(APIS.VEHICLE_GROUP, id)
        .then((res) => {
          const editData = res?.data?.vehicleGroup;
          setData((prev) => ({
            ...prev,
  ...
  const handleSubmit = (values, { setSubmitting, resetForm }) => {
    ...
    if (id) {
      payload._id = id;
      patchApi(APIS.VEHICLE_GROUP, id, payload).then(() => {
        toast.success("Data updated successfully");
        navigate(-1);
      });
    } else {
      postApi(APIS.VEHICLE_GROUP, payload)
        .then(() => {
            ...
            {loading ? (
              ...
              <>
                <Header>
                  <div>
                    <BreadCrumbs
                      ...
                      boldItem={"Vehicle Group"}
                    />
                    <Heading>{id ? "Edit" : "Add"} Vehicle Group</Heading>
                  </div>
                  <ButtonContainer>
                    <Button
                ...
                <Content>
                  <Section>
                    <SidePanel
                      title={`Add Vehicle Group (En)`}
                      description={`Enter Vehicle Group Detail`}
                    />
                    <InputsContainer>
                      ...
                      <div className=" pb-4 md:w-1/3">
                        <FormikTextField
                          label="Vehicle Group Short Name"
                          placeholder="Enter Vehicle Group Short Name"
...
export default AddEditVehicleGroup;
