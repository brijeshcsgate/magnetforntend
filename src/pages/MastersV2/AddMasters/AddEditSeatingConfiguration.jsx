...
const AddEditSeatingConfiguration = () => {
  ...
  useEffect(() => {
    if (id) {
      setLoading(true);
      getApi(APIS.SEATING_CONFIGURATION, id)
        .then((res) => {
          const editData = res?.data?.seatingConfiguration;
          setData((prev) => ({
            ...prev,
  ...
  const handleSubmit = (values, { setSubmitting, resetForm }) => {
    ...
    if (id) {
      payload._id = id;
      patchApi(APIS.SEATING_CONFIGURATION, id, payload).then(() => {
        toast.success("Data updated successfully");
        navigate(-1);
      });
    } else {
      postApi(APIS.SEATING_CONFIGURATION, payload)
        .then(() => {
            ...
            {loading ? (
              ...
              <>
                <Header>
                  <div>
                    <BreadCrumbs
                      ...
                      boldItem={"Seating Configuration"}
                    />
                    <Heading>
                      {id ? "Edit" : "Add"} Seating Configuration
                    </Heading>
                  </div>
                  <ButtonContainer>
                    <Button
                ...
                <Content>
                  <Section>
                    <SidePanel
                      title={`Add Seating Configuration (En)`}
                      description={`Enter Seating Configuration Detail`}
                    />
                    <InputsContainer>
                      <div className="flex gap-8">
                        <div className=" pb-4 md:w-1/3">
                          <FormikTextField
                            label="Seating Configuration Name"
                            placeholder="Enter Seating Configuration Name"
                            name="englishName"
                            isRequired={true}
                          />
                        </div>
                      ...
                      <div className=" pb-4 md:w-1/3">
                        <FormikTextField
                          label="Seating Configuration Short Name"
                          placeholder="Enter Seating Configuration Short Name"
                  ...
                  <Section>
                    <SidePanel
                      title={`Add Seating Configuration (Hi)`}
                      description={`Enter Seating Configuration Detail`}
                    />
...
export default AddEditSeatingConfiguration;
