...
const AddEditTyreType = () => {
  ...
  useEffect(() => {
    if (id) {
      setLoading(true);
      getApi(APIS.TYRE_TYPE, id)
        .then((res) => {
          const editData = res?.data?.tyreType;
          setData((prev) => ({
            ...prev,
  ...
  const handleSubmit = (values, { setSubmitting, resetForm }) => {
    ...
    if (id) {
      payload._id = id;
      patchApi(APIS.TYRE_TYPE, id, payload).then(() => {
        toast.success("Data updated successfully");
        navigate(-1);
      });
    } else {
      postApi(APIS.TYRE_TYPE, payload)
        .then(() => {
            ...
            {loading ? (
              ...
              <>
                <Header>
                  <div>
                    <BreadCrumbs
                      ...
                      boldItem={"Tyre Type"}
                    />
                    <Heading>{id ? "Edit" : "Add"} Tyre Type</Heading>
                  </div>
                  <ButtonContainer>
                    <Button
                ...
                <Content>
                  <Section>
                    <SidePanel
                      title={`Add Tyre Type (En)`}
                      description={`Enter Tyre Type Detail`}
                    />
                    <InputsContainer>
                      <div className="flex gap-8">
                        <div className=" pb-4 md:w-1/3">
                          <FormikTextField
                            label="Tyre Type Name"
                            placeholder="Enter Tyre Type Name"
                            name="englishName"
                            isRequired={true}
                          />
                        </div>
                      ...
                      <div className=" pb-4 md:w-1/3">
                        <FormikTextField
                          label="Tyre Type Short Name"
                          placeholder="Enter Tyre Type Short Name"
...
export default AddEditTyreType;
